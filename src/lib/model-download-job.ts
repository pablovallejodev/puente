...
export function makeDownloadTaskId(modelId: string, relativePath: string): string {
  return `${DOWNLOAD_TASK_PREFIX}${modelId}:${relativePath}`;
}

export function parseDownloadTaskId(taskId: string): { modelId: string; relativePath: string } | null {
...
export function createDownloadJobState(modelId: string, opts?: { selectOnComplete?: boolean }): DownloadJobState {
  return {
    modelId,
    status: 'active',
    completedFiles: [],
...
export function parseDownloadJobState(raw: string): DownloadJobState | null {
  try {
    ...
    return {
      ...
      completedFiles: parsed.completedFiles.filter((f): f is string => typeof f === 'string'),
      currentFile: typeof parsed.currentFile === 'string' ? parsed.currentFile : null,
      bytesWritten: typeof parsed.bytesWritten === 'number' && parsed.bytesWritten >= 0 ? parsed.bytesWritten : 0,
