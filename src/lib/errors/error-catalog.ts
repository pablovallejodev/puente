...
const MODEL_DOCS: Record<string, ErrorDoc> = {
  MODEL_UNKNOWN_ID: {
    ...
    cause: 'Una preferencia guardada apunta a un modelo retirado del catálogo, o el id está mal escrito en el código.',
  ...
  MODEL_NOT_INSTALLED: {
    ...
    cause: 'Falta el marcador .complete: la descarga nunca terminó o los ficheros se borraron desde fuera de la app.',
    fix: 'Descarga el modelo desde Configuración.',
  },
  MODEL_INCOMPLETE: {
    summary: 'Falta algún fichero del modelo instalado.',
    cause: 'La carpeta se borró parcialmente, o el catálogo añadió un fichero nuevo a un modelo ya instalado.',
  ...
  MODEL_DOWNLOAD_HTTP: {
    ...
    cause: '401/403 en un repositorio con licencia aceptada manualmente, 404 si la ruta cambió, 5xx si el CDN falla.',
  ...
  MODEL_FINALIZE_FAILED: {
    ...
    cause: 'Otro proceso tenía la carpeta abierta, o el sistema de ficheros rechazó el move.',
  ...
  MODEL_PREFS_READ_FAILED: {
    ...
    cause: 'expo-secure-store no está disponible, o el llavero del dispositivo está bloqueado.',
  ...
  MODEL_ENGINE_PATH_MISSING: {
    ...
    cause: 'El directorio de documentos no está disponible, o el fichero desapareció entre la verificación y la carga.',
...
const SHARED_ORT_DOCS: Record<string, ErrorDoc> = {
  ...
  TOKENIZER_LOAD_FAILED: {
    ...
    cause: 'tokenizer.json o tokenizer_config.json corrupto o incompatible con @huggingface/tokenizers.',
  ...
  ORT_NOT_REGISTERED: {
    ...
    cause: 'Se instalaron dependencias nativas nuevas sin regenerar el proyecto nativo.',
    fix: 'pnpm install && npx expo prebuild --clean, y reconstruye la app.',
  },
  LANGUAGE_UNSUPPORTED: {
    summary: 'El modelo no admite ese idioma.',
    cause: 'El par origen/destino no está en el vocabulario del modelo activo. Cada modelo cubre un conjunto distinto.',
  ...
  DECODE_FAILED: {
    ...
    cause: 'Desajuste en la caché KV, o memoria insuficiente a mitad de la generación.',
  ...
  DECODE_EMPTY: {
    ...
    cause: 'Entrada vacía tras el filtrado, o el modelo emitió fin de secuencia de inmediato.',
  ...
  OUT_OF_MEMORY: {
    ...
    cause: 'Android mató la asignación. Otras apps en segundo plano reducen mucho la memoria realmente disponible.',
...
const WHISPER_DOCS: Record<string, ErrorDoc> = {
  ...
  LANG_DETECT_EMPTY: {
    ...
    cause: 'generation_config.json sin lang_to_id, típico de un modelo Whisper solo-inglés usado en modo Universal.',
  ...
  LANG_DETECT_UNSUPPORTED: {
    ...
    cause: 'Whisper reconoce más idiomas de los que la app puede traducir después.',
...
const ENGINE_DOCS: Record<string, ErrorDoc> = {
  ...
  ENGINE_INIT_FAILED: {
    ...
    cause: 'Ficheros que el motor nativo no reconoce, o memoria insuficiente al cargar los pesos.',
...
export function describeCode(domain: DiagnosticDomain, code: string): ErrorDoc | undefined {
