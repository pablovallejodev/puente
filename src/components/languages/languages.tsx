...
import { TRADUCTOR_LANGUAGES, useTraductorSession } from '@/contexts/traductor-session-context';
...
function matchesQuery(language: TraductorLanguage, query: string, uiLocale: UiLocale): boolean {
  if (!query) return true;
  const q = query.trim().toLowerCase();
  return (
    getTraductorLanguageDisplayName(language, uiLocale).toLowerCase().includes(q) ||
...
export default function LanguagesComponent() {
  ...
  const uiLocale = useMemo(() => resolveUiLocale(getDeviceLocaleTag()), []);
  ...
  const recommendedAll = useMemo(() => getRecommendedLanguages(getDeviceLocaleTag()), []);
  const recommendedIds = useMemo(() => new Set(recommendedAll.map((lang) => lang.id)), [recommendedAll]);

  const visibleRecommended = useMemo(
    () => recommendedAll.filter((lang) => !blockedIds.has(lang.id) && matchesQuery(lang, query, uiLocale)),
  ...
  const filteredLanguages = useMemo(
    () =>
      TRADUCTOR_LANGUAGES.filter(
        (lang) => !recommendedIds.has(lang.id) && !blockedIds.has(lang.id) && matchesQuery(lang, query, uiLocale),
      ),
    [blockedIds, query, recommendedIds, uiLocale],
  );

  const showUniversal =
    slot === 'input' && mode === 'one_way' && (!query.trim() || 'universal'.includes(query.trim().toLowerCase()));

  const sections = useMemo(() => [{ title: '', data: filteredLanguages }], [filteredLanguages]);
  ...
  const isLanguageSelected = (language: TraductorLanguage) => {
    ...
    return inputLanguage.kind === 'fixed' && inputLanguage.language.id === language.id;
  };
    ...
    return (
      ...
      >
        ...
        <View style={styles.universalLabels}>
          ...
          <Text style={styles.universalHint}>Whisper detecta el idioma automáticamente</Text>
        </View>
