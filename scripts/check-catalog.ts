...
function testFiles(): void {
  for (const spec of ALL_MODELS) {
    ...
    for (const file of spec.files) {
      assert.ok(!relativePaths.has(file.relativePath), `${spec.id}: two files land on ${file.relativePath}`);
      ...
      assert.ok(!file.relativePath.includes('/'), `${spec.id}: nested relativePath ${file.relativePath}`);
      assert.ok(
      ...
      assert.ok(file.expectedBytes > 0, `${spec.id}: ${file.relativePath} has no expected size`);
    }
...
function testRuntimeFilesExist(): void {
  const filesOf = (spec: ModelSpec) => new Set(spec.files.map((f) => f.relativePath));
  ...
  for (const spec of ALL_MODELS) {
    ...
    switch (runtime.kind) {
      ...
      case 'offline-asr':
        ...
        assert.ok(
          available.has('tokens.txt') || [...available].some((f) => f.endsWith('tokens.txt')),
    ...
    for (const file of required) {
      assert.ok(available.has(file), `${spec.id}: runtime needs ${file}, which is not downloaded`);
    }
  }
}
...
function testMetadata(): void {
  ...
  for (const spec of ALL_MODELS) {
    ...
    assert.ok(RAM_TIERS.includes(spec.ramTier), `${spec.id}: ramTier must be 1–4`);
    assert.ok(
  ...
  const tiers = [RAM_TIER.entry, RAM_TIER.low, RAM_TIER.mid, RAM_TIER.high, RAM_TIER.flagship];
...
function testModesAndPairBudget(): void {
  ...
  assert.equal(resolveActiveMode('sherpa-whisper-turbo-int8', 'nllb-600m-q8'), 'universal');
  assert.equal(resolveActiveMode('sherpa-whisper-turbo-int8', 'salamandrata-2b-instruct-q4'), 'europeo');
  assert.equal(resolveActiveMode('sherpa-sense-voice-multi-int8', 'nllb-600m-q8'), 'asiatico');
  assert.equal(resolveActiveMode('whisper-base-q', 'nllb-600m-q8'), 'bajos-recursos');
  assert.equal(resolveActiveMode('whisper-tiny-q', 'nllb-600m-q8'), 'personalizado');
  ...
  assert.equal(exceedsPairBudget(requireModelSpec('sherpa-whisper-turbo-int8').peakRamBytes, nllb, 8 * GB), true);
