...
export function useSpeechTranslator() {
  ...
  const processQueue = useCallback(async () => {
    ...
    try {
      while (queueRef.current.length > 0) {
        ...
        const mappedLocale = utterance.locale ? mapSpeechLocaleToFlores(utterance.locale) : null;
        const srcLang = mappedLocale ?? mapSpeechLocaleToFlores(getDeviceLocaleTag()) ?? 'eng_Latn';
  ...
  const startRecognition = useCallback(async () => {
    if (Platform.OS === 'web') {
      setErrorMessage('Speech recognition requires a development build on Android or iOS.');
      return;
    }
    ...
    if (!permissions.granted) {
      setErrorMessage('Microphone permission is required to listen for speech.');
      return;
    }
